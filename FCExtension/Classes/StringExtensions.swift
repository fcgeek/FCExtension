...
public extension String {
    ...
    public var isHttpsUrl: Bool {
        guard lowercased().start(with: "https://".lowercased()) else {
            return false
        }
    ...
    public var isHttpUrl: Bool {
        guard lowercased().start(with: "http://".lowercased()) else {
            return false
        }
        return URL(string: self) != nil
    }
    ...
    public var lastCharacter: String? {
        return Array(self).map({String($0)}).last
    }
...
public extension String {
    ...
    public mutating func truncate(to length: Int, trailing: String? = "...") {
        if self.count > length {
            self = self.prefix(length) + (trailing ?? "")
        }
    }
